...
$(() => {
    ...
    ***********************************************/
    ...
    chkall.change(function(){
        ...
        // 2. 전체 체크박스가 체크상태(turn)이면 개별 체크 박스도 모두 turn으로 
        // 체크상태 변경
        ...
        -> 둘의 차이는 일반 속성값을 읽어올때는 차이없음!
        체크박스의 checked 속성인 경우 turn/false를 리턴해 주는 것은
        prop()메서드 뿐!
    ...
    ***********************************************/
    // 원리: 개별 체크박스가 모두 체크되면 전체 체크박스 체크하기
    // 대상: .chk->chkeach 변수
    ...
    ***********************************************/
    ...
    $(".YNbox button").click(function(){
        ...
        // 2. 동의버튼 클릭일 경우 : 필수버튼 확인 후 회원가입 허가
        if(isB){
            // (1) 필수항목이 모두 체크된 경우
            if(
 ...
 ********************************************/
    ...
    $("#btnj").click((e) => {
        ...
        // 4. 검사결과에 따라 메시지 보이기
        if(pass){
            // 원래는 post 방식으로 DB에 회원가입정보를 전송하여 입력후
            // DB처리완료시 성공메시지나 로그인 페이지로 넘겨준다!
            alert("회원가입을 축하드립니다!");
            // 로그인 페이지로 리디렉션
