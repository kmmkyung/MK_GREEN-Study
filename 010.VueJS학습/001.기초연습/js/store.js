...
/************************************************************
    [ Vuex Store - 뷰엑스 스토어란? ]
    1. 데이터 데이터 상태를 한번에 관리하는 확장 라이브러리다!
    2. 스토어에서 리리되는 데이터는 리엑티브 데이터 이다!
        (reactive data 양방향 동기화 데이터이다!)
    3. 컴포넌트 구조 상태와 상관없이 사용하는 곳에서 변경한 내용이
        동기화되어 자연스런 업데이트가 이루어진다!

    [  뷰엑스 스토어 기본구조 ]
    1. state (상태) : 변수 선언 및 할당 구역(인스턴스)
        => 뷰 인스턴스에서 data와 유사
    ...
    - 흐름: 먼저 비동기 메서드 처리후 일반 메서드 처리 결과를 state변수에 반영함!
    (참고: 뷰엑스 관리 흐름도
    ...
    2. mutations 내부 메서드 호출: commit("메서드명",전달변수)
        => actions 와 mutations 내부의 메서드는 다음과 같은 형태로 구성된다
        actions/mutations:{
            메서드명(첫변수,둘째변수){

            } -> 첫 변수는 mutation은 뷰엑스 스토어의 state 변수가 자동으로 들어옴!
    ...
    (예컨데 최신 뷰는 3.x 이므로 vuex는 4.x버전을 씀)
