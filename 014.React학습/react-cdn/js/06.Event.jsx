...
function EventShow(){
    ...
    const aladin = (lamp) => { // lamp - 알라딘 주인공 이미지 경로!
        console.log("alain 함수",lamp);

        // 1. #wrap 인 요소의 내부에 h1 요소 넣기
        document.querySelector("#tbox").innerHTML +=
        `<h1 class="tit">소원이 무엇이냐</h1>`

        // 2. 소원이 무엇이냐 타이틀 박스 CSS 구성하기
        ...
        // 4. 별도의 요소인 #ala에 이미지 생성 출력하기 컴포넌트
    ...
    return(
        <React.Fragment>
            ...
            <button onClick={()=>{getIt2
            ("https://www.pngplay.com/wp-content/uploads/13/Ferrari-458-Transparent-PNG.png");
            }}>소원빌기~ 페라리주세요!</button>
...
////////////// 알라딘 램프 이미지 출력 컴포넌트
function AlaLamp(props){ // 이미지 경로를 props 로 받는다!
...
function Ferrari(props){
    return (
        <img id="car" src={props.isrc} alt="페라리" title="클릭하면 시운전해요"
        onClick={move} />
    )
} //////////// Ferrari ///////////////
