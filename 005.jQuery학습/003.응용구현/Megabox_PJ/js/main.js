...
$(function () {
    ...
    // 변경대상: .tbar
    const tBar = $(".tBar") 
    // 사용할 이벤트 : timeupdate(비디요 요소의 시간이 변경될때 발생)
    // on(이벤트,함수) 메서드 사용!
    mv.on("timeupdate",function(){
        // 목표 : 비디오가 재생중일때 진행바가 움직이게 한다
        // 원리 : 진행바의 진행비율로 %로 나타내야한다!
        // 계산법 : 현재 시간 / 전체 시간 * 100 => 백분율(%)

        // 1. 비디오 현재시간 가져오기
        let ctime = mv[0].currentTime;
        // mv[0] === mv.get(0)
        // 비교) document.querySelectoerAll("my").item(0)
        //      === document.querySelectoerAll("my")[0]
        ...
        // 현재 시간이 0일 경우 전체시간이 안나옴(if문으로 막기)
        if(!isNaN(ftime)){ //ftime이 숫자가 아니면!! 
            let percent = ctime / ftime * 100
            console.log('진행율',percent.toFixed(2));
            // 소수점변수.foFixed(자릿수) -> 특정자릿수까지만 소수점 표시!
