...
$(() => {
    ...
    *************************************/ 
        ...
        slide.on("dragstop",function(){
            ...
            // 슬라이드의 left값(#viewer>ul 의 left값)
            let spos = slide.position().left;
            // offset().left은 전체 화면을 기준한 left값
            // !!!--- position().left는 부모자격박스를 기준한 left값 ---!!!
            // 여기서 부모자격박스느 #viewer임!
            console.log("슬라이드 left",spos);

            // (1) 왼쪽 방향일때 -> 오른쪽 버튼 클릭 시
            if(spos < -50) $(".rb").trigger("click");
            // (2) 오른쪽 방향일때 -> 왼쪽 버튼 클릭 시
