...
function loadFn(){
    ...
    // 잘라내기로 li순번이 뒤섞이므로 블릿변경 매칭을 위한 고유순번을 사용자정의 속성(data-)으로 만들어준다!
    ...
    const goSlide = (seq) => {
        ...
        // 1-2. 왼쪽 버튼 클릭시
            ...
            // 나타나도록 하기위해 우선 맨뒤 li를
            // 맨앞으로 이동한다.
            // slide.insertBefore(넣을넘,넣을놈전놈);
            // slide.insertBefore(맨끝li, 맨앞li);
            slide.insertBefore(clist[clist.length-1],clist[0]);

            // (2) 동시에 top값을 -100%으로 변경한다!
            slide.style.top="-100%";
            // 이때 트랜지션을 없앤다(한번 실행 후 부터 생기므로!)
            slide.style.transition="none";

            // (3) 그 후 top값을 0으로 애니메이션하여
            // 슬라이드가 왼쪽에서 들어온다.
            // 동일 속성인 top가 같은 코딩처리 공간에 동시에 있으므로 이것을 분리해야 효과가 있다
            // setTimeout을 사용한다!
            setTimeout(()=>{
                slide.style.top= "0";
                slide.style.transition= "top .4s ease-in-out"
        ...
        // 2-3. 읽어온 슬라이드 순번의 블릿에 클래스 "on"넣기
