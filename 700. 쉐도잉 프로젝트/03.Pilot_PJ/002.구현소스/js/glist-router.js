...
// v-for에 기술된 v-if 조건에 사이범위를 넣고 스토어 변수로 컨트롤한다!
...
let Paging = {
    ...
    <section>
        ...
        <div id="paging">
        <a v-for="v in ">
            <a href="#" @click.prevent="$store.commit('updatePageing',0)">
            1
            </a>
            |
            <a href="#" @click.prevent="$store.commit('updatePageing',10)">
            2
            </a>
            |
            <a href="#" @click.prevent="$store.commit('updatePageing',20)">
            3
            </a>
        </a>
        </div>
...
<section>
    ...
    <div class="grid">
        <div v-for="(v,i) in $store.state.gdata"
        v-if="
        v.idx >= 1 && v.idx<=10 + $store.state.mnum
        "
        >
        [{{v.idx}}]<img v-bind:src="
            './images/goods/'+
        ...
        <!-- 모어 버튼 표시구역 -->
        <div id="more">
        <button class="more" @click.prevent="$store.commit('updateMore',30)" v-if="$store.state.mbtn">
            MORE
        </button>
        </div>
    </section>
